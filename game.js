...
awe6_core_drivers_createjs_Factory.prototype = $extend(awe6_core_drivers_AFactory.prototype,{
	...
	,_getAssetUrls: function() {
		var l_result = ["bin/assets/Blank.png","bin/assets/Circle.png","bin/assets/fonts/__Orbitron.eot","bin/assets/fonts/__Orbitron.svg","bin/assets/fonts/__Orbitron.ttf","bin/assets/fonts/__Orbitron.woff","bin/assets/normalmapping/normal.png","bin/assets/normalmapping/texture.png","bin/assets/__Config.xml","bin/assets/__Icon128.png","bin/assets/__Icon196.png","bin/assets/__Icon256.png"];
		var l_toRemove = [];
		var _g1 = 0;
		var _g = l_result.length;
		while(_g1 < _g) {
...
var canvascandy_Utils = function() { };
$hxClasses["canvascandy.Utils"] = canvascandy_Utils;
canvascandy_Utils.__name__ = true;
canvascandy_Utils.getImage = function(p_url) {
	var l_result = window.document.createElement("img");
	l_result.src = p_url;
	return l_result;
};
canvascandy_Utils.createCanvas = function(p_width,p_height) {
	var l_result = window.document.createElement("canvas");
	l_result.width = p_width;
	l_result.height = p_height;
	return l_result;
};
canvascandy_Utils.createCanvasFromImage = function(p_image) {
	var l_result = canvascandy_Utils.createCanvas(p_image.width,p_image.height);
	l_result.getContext("2d",null).drawImage(p_image,0,0);
	return l_result;
};
var canvascandy_normalmapping_NormalMapping = function(p_kernel,p_texture,p_normal,p_isAutoLight,p_ambient,p_spotlight,p_shine) {
	if(p_shine == null) p_shine = .5;
	if(p_spotlight == null) p_spotlight = .5;
	if(p_ambient == null) p_ambient = .5;
	if(p_isAutoLight == null) p_isAutoLight = true;
	this._context = new createjs.Container();
	this._texture = p_texture;
	this._normal = p_normal;
	this._isAutoLight = p_isAutoLight;
	this._ambient = p_ambient;
	this._spotlight = p_spotlight;
	this._shine = p_shine;
	awe6_core_Entity.call(this,p_kernel,null,this._context);
};
$hxClasses["canvascandy.normalmapping.NormalMapping"] = canvascandy_normalmapping_NormalMapping;
canvascandy_normalmapping_NormalMapping.__name__ = true;
canvascandy_normalmapping_NormalMapping.__super__ = awe6_core_Entity;
canvascandy_normalmapping_NormalMapping.prototype = $extend(awe6_core_Entity.prototype,{
	_init: function() {
		awe6_core_Entity.prototype._init.call(this);
		this._ambient = this._tools.limit(this._ambient,0,1);
		this._spotlight = this._tools.limit(this._spotlight,0,1) * 2;
		this._shine = 50 * this._tools.limit(this._shine,.1,1);
		this._width = this._texture.width;
		this._height = this._texture.height;
		this._context.cache(0,0,this._width,this._height);
		this._canvas = this._context.cacheCanvas;
		this._context2d = this._canvas.getContext("2d",null);
		this._buffer = this._context2d.getImageData(0,0,this._width,this._height);
		this._normalData = this._precalculateNormalData(this._normal);
		this._textureData = this._getDataFromImage(this._texture).data;
		this._spotlightX = 0;
		this._spotlightY = 0;
		this._spotlightZ = Math.round(Math.sqrt(this._width * this._width + this._height * this._height) / 4);
		this.configure(.5,.5);
	}
	,_updater: function(p_deltaTime) {
		if(p_deltaTime == null) p_deltaTime = 0;
		awe6_core_Entity.prototype._updater.call(this,p_deltaTime);
		if(this._isAutoLight) this.configure(this._kernel.inputs.mouse.relativeX,this._kernel.inputs.mouse.relativeY);
		this._draw();
	}
	,configure: function(p_lx,p_ly) {
		var l_x = Math.round(p_lx * this._width);
		var l_y = Math.round(p_ly * this._height);
		this._isDirty = this._spotlightX != l_x || this._spotlightY != l_y;
		this._spotlightX = l_x;
		this._spotlightY = l_y;
	}
	,_getDataFromImage: function(p_image) {
		return ((function($this) {
			var $r;
			var _this = canvascandy_Utils.createCanvasFromImage(p_image);
			$r = _this.getContext("2d",null);
			return $r;
		}(this))).getImageData(0,0,p_image.width,p_image.height);
	}
	,_precalculateNormalData: function(p_normal) {
		var l_result = [];
		var l_data = this._getDataFromImage(this._normal).data;
		var l_nx;
		var l_ny;
		var l_nz;
		var l_magInv;
		var l_i = 0;
		var l_max = this._width * this._height * 4;
		while(l_i < l_max) {
			l_nx = l_data[l_i];
			l_ny = 255 - l_data[l_i + 1];
			l_nz = l_data[l_i + 2];
			l_magInv = 1 / Math.sqrt(l_nx * l_nx + l_ny * l_ny + l_nz * l_nz);
			if(!isFinite(l_magInv) || l_magInv > 1000) l_magInv = 1000;
			l_nx = l_nx * l_magInv * 256 | 0;
			l_ny = l_ny * l_magInv * 256 | 0;
			l_nz = l_nz * l_magInv * 256 | 0;
			l_result.push(l_nx);
			l_result.push(l_ny);
			l_result.push(l_nz);
			l_i += 4;
	...
	,_clampInt: function(p_value,p_min,p_max) {
	...
	,_draw: function() {
		...
		if(!this._kernel.isEyeCandy) return;
		var l_data = this._buffer.data;
		var l_i = 0;
		var l_ni = 0;
		var l_nx = 0;
		var l_ny = 0;
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var l_x = _g3++;
				l_nx = this._normalData[l_ni];
				l_ny = this._normalData[l_ni + 1];
				l_nz = this._normalData[l_ni + 2];
				if(this._spotlight > 0 || (l_ni & 1) == 0) {
					l_dx = this._spotlightX - l_x;
					l_dy = this._spotlightY - l_y;
					l_dz = this._spotlightZ;
					l_magInv = 256 / Math.sqrt(l_dx * l_dx + l_dy * l_dy + l_dz * l_dz);
					l_dx = l_dx * l_magInv | 0;
					l_dy = l_dy * l_magInv | 0;
					l_dz = l_dz * l_magInv | 0;
					l_dotProduct = l_dx * l_nx + l_dy * l_ny + l_dz * l_nz;
					l_intensity = Math.pow(l_dotProduct / 65536,this._shine) * this._spotlight + this._ambient;
				}
				var _g4 = 0;
				while(_g4 < 3) {
					var l_channel = _g4++;
					l_data[l_i + l_channel] = Math.round(this._clampInt(this._textureData[l_i + l_channel] * l_intensity | 0,0,255));
				}
				l_data[l_i + 3] = this._textureData[l_i + 3];
				l_i += 4;
				l_ni += 3;
			}
		}
		this._context2d.putImageData(this._buffer,0,0);
		this._isDirty = false;
	}
	,__class__: canvascandy_normalmapping_NormalMapping
});
...
demo_Factory.prototype = $extend(awe6_core_drivers_createjs_Factory.prototype,{
	...
	,createScene: function(p_type) {
		return new demo_scenes_SceneNormalMapping(this._kernel,p_type,true,true);
	}
	,createTextStyle: function(p_type) {
		return new awe6_core_TextStyle(this._kernel.getConfig("settings.font.name"),20,16777215,null,null,awe6_interfaces_ETextAlign.CENTER);
	}
	,__class__: demo_Factory
});
var demo_scenes_AScene = function(p_kernel,p_type,p_isPauseable,p_isMuteable,p_isSessionSavedOnNext) {
	if(p_isSessionSavedOnNext == null) p_isSessionSavedOnNext = false;
	if(p_isMuteable == null) p_isMuteable = true;
	if(p_isPauseable == null) p_isPauseable = false;
	awe6_core_Scene.call(this,p_kernel,p_type,p_isPauseable,p_isMuteable,p_isSessionSavedOnNext);
};
$hxClasses["demo.scenes.AScene"] = demo_scenes_AScene;
demo_scenes_AScene.__name__ = true;
demo_scenes_AScene.__super__ = awe6_core_Scene;
demo_scenes_AScene.prototype = $extend(awe6_core_Scene.prototype,{
	_init: function() {
		awe6_core_Scene.prototype._init.call(this);
		this._text = new awe6_core_drivers_createjs_extras_gui_Text(this._kernel,this._kernel.factory.width,30,"",this._kernel.factory.createTextStyle());
		this._text.setPosition(0,this._kernel.factory.height - this._text.height);
		this.addEntity(this._text,null,true,10);
	}
	,__class__: demo_scenes_AScene
});
var demo_scenes_SceneNormalMapping = function(p_kernel,p_type,p_isPauseable,p_isMuteable,p_isSessionSavedOnNext) {
	demo_scenes_AScene.call(this,p_kernel,p_type,p_isPauseable,p_isMuteable,p_isSessionSavedOnNext);
};
$hxClasses["demo.scenes.SceneNormalMapping"] = demo_scenes_SceneNormalMapping;
demo_scenes_SceneNormalMapping.__name__ = true;
demo_scenes_SceneNormalMapping.__super__ = demo_scenes_AScene;
demo_scenes_SceneNormalMapping.prototype = $extend(demo_scenes_AScene.prototype,{
	_init: function() {
		demo_scenes_AScene.prototype._init.call(this);
		this.addEntity(new canvascandy_normalmapping_NormalMapping(this._kernel,canvascandy_Utils.getImage("assets/normalmapping/texture.png"),canvascandy_Utils.getImage("assets/normalmapping/normal.png")),null,true,2);
		this._text.set_text("NormalMapping");
	}
	,__class__: demo_scenes_SceneNormalMapping
});
